...
- `resume_id` (uuid, foreign key to resumes) - Resume that was analyzed
...
- `resume_id` (uuid, foreign key to resumes) - Source resume
...
- `id` (uuid, primary key) - Unique identifier
- `user_id` (uuid, foreign key to auth.users) - Owner
- `analysis_id` (uuid, foreign key to job_analyses) - Related analysis
- `plan_content` (jsonb) - Structured job search plan
- `created_at` (timestamptz) - Creation timestamp

### user_credits
- `id` (uuid, primary key) - Unique identifier
- `user_id` (uuid, foreign key to auth.users, unique) - Owner
...
- Each authenticated user can only access their own records
- All tables use DEFAULT auth.uid() for user_id columns
...
-- Function to automatically create user_credits for new users
